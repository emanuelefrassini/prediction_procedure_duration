...
import keras
from keras.layers import  Layer,LSTM,Bidirectional, Dropout, Dense,Input, multiply, concatenate, Activation, Masking, Reshape,Conv1D, BatchNormalization, GlobalAveragePooling1D
...
elif pc=='PC_mine' or pc=='PC_office':
...
X=np.zeros((228,7573,2))
 
for j in range(len(loaded_data)):
    X[j,:,0]=loaded_data[j]["Phase_num"]
    X[j,:,1]=loaded_data[j]["CumTime"]
    
for i in range(X.shape[0]):
    h=0
    X[i,0,1]=1
    while h<7573 and X[i,h,1]!=0:
        X[i,h,1]=h+1
        h+=1   
duration=[]
for i in range(X.shape[0]):
    if len(np.where(X[i, :, 1] == 0)[0])>0:
        duration.append(np.where(X[i, :, 1] == 0)[0][0])
    else: duration.append(7573)

Y=np.zeros((X.shape[0],X.shape[1],1))
for i in range(Y.shape[0]):
    for j in range(duration[i]):
        Y[i,j,0]=duration[i]-j-1
    for j in range(duration[i],7573):
        X[i,j,0]=-1
        Y[i,j,0]=-1

X=X[:,:,[0]]

count_14_per_row_insec = Y[:,0]
...
Y = np.delete(Y, indices_less, axis=0)
X = np.delete(X, indices_less, axis=0)

if granularity=='min':
    X[(X == 1) | (X == 2)] = 0  # C\A and C\B becomes C (0)
    X[(X == 3) | (X == 4) | (X == 5) | (X == 6)] = 1  # D,E,F,Fa become M_first (1)
    X[(X == 7) | (X == 8) | (X == 9) | (X == 10)] = 2  # G,H,Ha,I become M_second (2)
    X[(X == 11) | (X == 12) | (X == 13)] = 3  # L\J,L\K,L become L (3)
    #X[(X == 14)] = 4 
...
def smape_loss(y_true, y_pred):
    weights = calculate_corridor_weights(
        y_pred,
        y_true,
        253,  # Using K.shape to get the shape of y_true
        2700,
        d=5,
        tolerance=0,
        off_corridor_penalty=1,
    ...
    smape_weighted = smape + weights
    return smape_weighted
...
HP_NUM_UNITS = hp.HParam('num_units', hp.Discrete([128,64,256,512]))
HP_DROPOUT = hp.HParam('dropout', hp.Discrete([0.1,0.2,0.3,0.4]))
#HP_OPTIMIZER = hp.HParam('optimizer', hp.Discrete(['adam', 'sgd']))
HP_LEARNINGRATE= hp.HParam('learning_rate', hp.Discrete([0.001,0.0001]))
HP_BATCHSIZE=hp.HParam('batch_size', hp.Discrete([2,4,8,16,32]))#,16,32,64]))
HP_REDUCE_DIM=hp.HParam('reduce_dim', hp.Discrete([5,10]))#,30,45,60]))
#HP_ATTENTION_UNITS=hp.HParam('attention_units', hp.Discrete([32,64,128]))
...
with tf.summary.create_file_writer(hyperparam_folder).as_default():
  hp.hparams_config(
    hparams=[HP_REDUCE_DIM,HP_NUM_UNITS, HP_DROPOUT,HP_LEARNINGRATE,HP_BATCHSIZE],
