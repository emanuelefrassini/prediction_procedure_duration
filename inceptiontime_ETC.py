...
def _inception_module(input_tensor, stride=1, activation='linear',batch_size=BATCH_SIZE,
                 nb_filters=32, use_residual=True, depth=6, kernel_size=41, nb_epochs=EPOCHS,use_bottleneck=True,
                 bottleneck_size = 32):
...
def create_model( input_shape=(X.shape[1],X.shape[2]),depth=6,use_residual=True):
...
smape_average=100
mae_average=100
while smape_average>SMAPE_MIN and mae_average>MAE_MIN:
    accuracy_per_slice = []
